---
title: "Robot - Data Description - v3"
...
```{r data, include=F, cache=T}
md <- read_twitter_csv(here("data", "clean_data", "202005-08", 
                            "all_clean_20210105_v3.csv")) %>% 
  mutate(robot = forcats::fct_relevel(robot, c("aibo", "lovot", "both")))
md_nort <- filter(md, is_retweet==FALSE)
```
...
md %>% 
  ...
  kable(caption="Tweets by Robot", align = c("lrr")) %>% 
...
md_nort %>% 
  ...
  kable_paper(full_width = F, position = "left") %>% 
  row_spec(4, bold=T) %>% 
  footnote(symbol = c("value: n (row %)",
                      "retweets are not counted"))
```


```{r hashtag_plot, fig.width=8}
# popular hashtags
tags <- md_nort %>% 
  filter(!hashtags=="NA") %>% 
